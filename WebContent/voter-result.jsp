...
<body onload = "func()" >
    ...
    try {
    	...
    	System.out.println("ON ELC " + elec);
    	...
    	for(String o :offs) {
    		voteCount=0;
    		sess.setAttribute("off", o);
    		sess.setAttribute("eecount",	(int)sess.getAttribute("eecount")+1);
    		...
    		for(Election es : els) {
    	 	...
    	 	for(Candidate can : cs){
    	 		...
    	 		if(can.getVoteCount()>voteCount) {
    	 			voteCount = can.getVoteCount();
    	 			winner = can.getName();
    	 			System.out.println("Winner " + winner);
    	    		sess.setAttribute("winner", winner);
    	    		%>
    	          	<script language="javascript">
    	          	var div = document.createElement('div');
    	          
    	         
    	              div.innerHTML = '  <label > Winner of ${off} election: </label> <br> '
    	              +'<div class="col-sm-6"> <label> ${winner} </label> </div> <br>';
    	              document.getElementById("elec" + ${eecount} ).appendChild(div);
